...
| [~] | macOS: ship `.app` bundle (fix Spotlight + terminal-detach) | New `scripts/make-macos-app.sh` wraps binary → `Downloader.app`; `release.yml`+`publish.sh` use it for `osx-*`; cask `app "Downloader.app"` v1.1.1; README/CONTRIBUTING updated. Verified detached launch locally. Needs v1.1.1 release + cask sha256 to go live. | _pending_ |

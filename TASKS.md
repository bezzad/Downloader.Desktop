...
| [x] | Issue 3: Instagram download R&D (plan only) | docs/instagram-rnd.md � recommend yt-dlp+ffmpeg behind a MediaExtractor (also unlocks YouTube/HLS); flagged ToS/legal; decision points for author. No code per author. | (doc) |
| [x] | Release v1.1.3 live via brew (Issues 1+2) | Tap cask �> 1.1.3 + real SHAs; brew upgrade 1.1.2�>1.1.3 verified (app reports 1.1.3). | (tap) |
| [x] | Round 16: About dialog + Donate + queue icons + Time Left + tri-state select-all + square toolbar buttons + progress-only recolor + README issues note + browser extension | `AboutView`/`AboutViewModel`+`DialogHelper.ShowAbout`, `Donate.md`, new icons, `DownloadsViewModel.SelectAllState`/`HasSelection`, `Button.tool`, `TimeLeftText`/`FormatDuration`, `StatusToBrush` on progress bar, `src/browser-extension` (Chrome/Edge+Firefox MV3), 16-locale keys. 92 tests green. **Extension store deploy + Safari deferred to author; non-English About strings machine-translated (review).** | develop |
| [x] | Round 15: shutdown-on-complete + queue start/stop + stop-all + granular notifications + 7 languages + browser listener + paste placeholder + Linux icon | `ShutdownService`, `BrowserIntegrationService`, manager `AllDownloadsCompleted`/`StopAll`(cancel), `DownloadsView` queue dropdowns, Notifications settings card, 7 new i18n packs (hi/it/pt/ru/zh/ja/ko) + keys in all 16, `install.sh`/`publish.sh`/`release.yml` icon. 83 tests green. **Browser extension still TBD (app side only).** | develop |
...
| [ ] | Use engine `RemoteFileResolver` for grid filename **+ size** (no start/stop) | Replace hand-rolled `Services/UrlResolver.ResolveFileNameAsync` with `RemoteFileResolver.GetFileInfoAsync(url)` (or `IDownloadService.GetFileInfoAsync`) → `RemoteFileInfo{FileName,FileSize,SupportsRange,Address}`; show size for queued items in the downloads Grid (`DownloadItemViewModel._previewName`, `DownloadsViewModel`). Keep `NameGate`/timeout wrapper. **BLOCKER:** bump `Downloader` PackageReference from 5.8.1 → first release that ships `RemoteFileResolver` (engine `develop` only today). | — |

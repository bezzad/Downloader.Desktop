...
| [!] | Submit winget PR to microsoft/winget-pkgs (v1.1.0) | **PR opened: microsoft/winget-pkgs#390226** (fork `bezzad/winget-pkgs`, branch `bezzad.Downloader-1.1.0`, 3 manifests via `gh api`). Blocked on author signing Microsoft CLA (comment `@microsoft-github-policy-service agree` on the PR), then their automated validation + review. | #390226 |

...
- **Per-fragment colors in the details strip** (#7): give `ChunkProgressViewModel` a stable `IBrush Brush` from a curated palette indexed by `Index` (no reshuffle on update), and bind the segment `ProgressBar Foreground="{Binding Brush}"` (the plain-track+fill ProgressBar's fill is its `Foreground`).

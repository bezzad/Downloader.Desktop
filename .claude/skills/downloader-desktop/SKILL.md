...
- **State transitions must be guarded in the manager, not just the buttons**: per-row buttons gate via `IsVisible`/`Can*`, but **bulk** actions (`StopSelected`→`Cancel`, `StartSelected`→`Resume`) apply to *every* selected row regardless of state. So the guards live in `DownloadManager` (the single choke point that all callers — buttons, bulk, scheduler, pump — go through): `Pause` no-ops unless Running; `Cancel` no-ops unless Running/Paused (else stopping a *completed* row flipped it to Stopped); `Resume`/`Start` no-op if Running/Completed; `Retry` only acts on Failed/Stopped. Without these, a completed download could be Stopped then re-`Start`ed from 0%, or a stray double-`Start` spun up a second engine reporting from 0% ("100% then begins again from 0"). Keep state-machine rules in the manager methods, not scattered across VMs/views.

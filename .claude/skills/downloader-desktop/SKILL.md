...
## The "test host hung" abort: read the dump on the OS that wrote it (2026-09-07)

Investigated properly for the first time, from the Windows hang dump of run 34016675882. **Do not
re-derive any of this**, and in particular do not start by reading the blamed test.

- **`dotnet-dump analyze` refuses a Windows/macOS dump on Linux** ("Analyzing Windows or MacOS dumps not
  supported when running on Linux"), which is why this was never analysed here before. Use
  `.github/workflows/analyze-hang-dump.yml`: it downloads a run's test-results artifact and prints
  `pstacks`, `clrthreads`, heap exception stats and `dumpasync` on a runner matching the dump's OS.
  `workflow_dispatch` 404s while the file is only on develop (dispatch resolves against the default
  branch), so it also has a push trigger scoped to itself — commit a new `DEFAULT_RUN_ID` to run it.
- **What the dump shows: 9 live threads, 3 roots, and NO Avalonia dispatcher/session thread.** The xunit
  worker is parked in `AvaloniaTestCase.Run` awaiting a `TaskCompletionSource` nothing will ever set;
  main is blocked in `XunitAutoGeneratedEntryPoint.Main`; everything else is thread-pool plumbing. No
  application code is executing anywhere.
- **The blamed test therefore never ran.** It is simply the next test to be dispatched, parked on a dead
...
- **The suite runs at `AvaloniaTestIsolationLevel.PerTest`** — the default when the assembly carries no
  ...
  two different ids for each. So the whole application and dispatcher are torn down and rebuilt ~1700
  times per run.
- **Avalonia's per-test teardown is unprotected.** In `HeadlessUnitTestSession.DispatchCore` the queued
  action ends with `finally { application.Dispose(); }` and the `tcs.TrySetResult/TrySetException` lines
  come AFTER that finally. Under PerTest, `application.Dispose()` is the entire app teardown
  (`ToolTipService`, `FontManager`, `Dispatcher.ResetForUnitTests`, locator scope). Anything that throws
  there leaves the completion source unset for ever, and the session loop catches only
  `OperationCanceledException`. That is the exact shape of this abort.
- **`AvaloniaTestIsolationLevel.PerAssembly` is NOT the fix — it was tried and it segfaults.** Full suite,
  Debug, coverlet settings: `Test process crashed with exit code 139` after 658 of ~1700 tests. Reverted.
  Do not propose it again without solving that crash first.

...
- **Auto-update** (`Services/UpdateService` + `UpdateFlow`): **version compare uses `Assembly.GetName().Version` = major.minor.patch (tracks `VersionPrefix`, AssemblyVersion is `major.minor.0.0`), NOT `InformationalVersion`** (that has the date-derived build/revision and would never compare sensibly to a `v1.1.0` tag). `UpdateService.IsNewer(tag, current)` + `Normalize(tag)` are pure/tested. Flow: GitHub `releases/latest` → if newer, in-app actionable toast (`NotificationService.ShowAction`, click = accept) → download the per-RID asset (`ExpectedAssetName()` matches release.yml names) via a throwaway `DownloadService` → `ApplyDownloadedArchive` spawns a detached unix `.sh`/win `.cmd` that waits for the PID to exit, extracts over the app dir, relaunches → `UpdateFlow.RequestShutdown` (= MainViewModel.Quit). The self-swap is untestable here; only the version logic has tests.
...
- **Rounded window corners**: set Window `Background="Transparent"` + `TransparencyLevelHint="Transparent"`, move the real background onto the root `Border` and give it `CornerRadius="16" ClipToBounds="True"`. Applied to all three windows. Caveat: on Linux WMs without a compositor the corners/shadow vary — accepted.

...
/// <summary>
/// Shared handling for the external tools this plugin provisions (the extractor, Deno, ffmpeg).
/// <para>
/// The rule this exists to enforce: a tool downloaded straight to its final path leaves a TRUNCATED file
/// behind when the app is closed or killed mid-download, and an existence-only "is it installed?" check
/// then treats that corpse as installed forever — the tool can never start and every extraction fails with
/// no way back (seen in the wild: a 23 MB fragment of a 40 MB yt-dlp, still without its executable bit,
/// months old). So downloads land on a temporary path and are moved into place only once complete, and
/// "installed" means present, plausibly sized AND runnable.
/// </para>
/// <para>
/// This mirrors the streaming-media plugin's <c>BinaryFile</c> rather than sharing it: the two plugins are
/// downloaded and installed independently, so neither may depend on the other's assembly being present.
/// </para>
/// </summary>
internal static class ToolFile
{
    ...
    /// <summary>Mark a downloaded tool runnable (no-op on Windows). In-process — spawning <c>chmod</c> can
    /// silently fail under sandboxes such as snap confinement.</summary>
    ...
    /// <summary>Find an executable on PATH, or null.</summary>

...
- [ ] 3b.3 **A download can hang for ever on a server that refuses every request.** Reproduced while writing the backoff test: with all its ranged requests refused, the engine sometimes emits NO completion event at all, so the row sits Running and nothing — not the backoff, not the failover, not the failure path — ever runs. The app cannot currently observe this. It needs a watchdog (no bytes and no event for N seconds ⇒ fail the attempt), which is a decision of its own rather than something to slip into this change. The missing happy-path test for the backoff ("a server that only tolerates one connection still downloads") is blocked on it — it hangs one run in three for exactly this reason.

...
- [ ] 3b.5 **After the engine ships the fix** (needs a NuGet release of `Downloader` ≥ 5.9.6): bump the app's package reference and re-enable the two `Skip`-ped tests in `Integration/UrlFailoverTests` — "a refused first address still downloads from the second" and "a server that only tolerates one connection". They are written and ready; with engine 5.9.5 they alternate between passing and waiting for an event that never comes, and the Skip reason names the upstream commit.

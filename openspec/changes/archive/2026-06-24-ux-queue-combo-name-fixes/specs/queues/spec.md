...
The "Start queue" and "Stop queue" toolbar dropdowns in the main downloads view SHALL list every queue that
currently exists, updating immediately when a queue is added or removed — without requiring an application
restart.

#### Scenario: New queue appears in the menus immediately

- **WHEN** the user adds a new queue from the Queues page
- **THEN** the new queue appears as an entry in both the "Start queue" and "Stop queue" toolbar dropdowns

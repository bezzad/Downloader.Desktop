...
- [x] 1.1 Confirm the starting state before writing the test:
      ...
      *Found: both at 1.7.0, no drift. Later bumped to 1.7.1 by another session (popup badge) and to
      1.8.0 by task 6.8 (identity handshake) — both manifests together, each time.*
- [x] 1.2 Add a test to `src/browser-extension/common.test.js` that reads both manifest files, parses them,
...
- [x] 1.3 Prove the test actually fails: temporarily change one manifest's version, run the test, see it go
      red naming both values, then revert. A guard test that has never been seen failing is not a guard.
- [x] 1.4 Run `node --test src/browser-extension/common.test.js`. Green.
- [x] 1.5 Add one line to `src/browser-extension/PUBLISHING.md` under the existing "bump both manifests"
      instruction, noting that a test now enforces it.
- [x] 1.6 Commit. This group touches only `src/browser-extension/`.

## 2. Publish a verifiable extension catalog with each release

- [x] 2.1 Read `scripts/build-plugins.sh` lines ~40–110 (the catalog-generation loop) — it is the template:
...
- [x] 2.2 Create `packaging/extension/targets.json` — the static, human-edited half of the catalog. One entry
...
- [x] 2.3 Extend `scripts/build-extension.sh`: after the two zips are built and `verify_zip`'d, read
...
- [x] 2.4 Run `./scripts/build-extension.sh` locally. Verify: both zips still build, `verify_zip` still
...
- [x] 2.5 In `.github/workflows/release.yml`, add `dist/extension-catalog.json` to the `files:` list of the
...
- [x] 2.6 Verify the workflow YAML still parses (`python3 -c "import yaml,sys;yaml.safe_load(open('.github/workflows/release.yml'))"`).
- [x] 2.7 Commit.

## 3. `BrowserDetector` — find installed browsers, read nothing else

- [x] 3.1 Add `src/Downloader.Desktop/Models/DetectedBrowser.cs`: a small record/POCO with `Id`, `Name`,
      `Family` (a new `BrowserFamily { Chromium, Gecko }` enum), `ExecutablePath`.
- [x] 3.2 Add `src/Downloader.Desktop/Services/BrowserDetector.cs`. Shape it like the other static,
...
- [x] 3.3 Hard-code the curated candidate table (see `design.md` → D3): Chrome, Chromium, Edge, Brave,
...
- [x] 3.4 Windows implementation: `Microsoft.Win32.Registry` only — walk
...
- [x] 3.5 Linux implementation: for each candidate executable name, probe each `PATH` entry plus the fixed
      list `/usr/bin`, `/usr/local/bin`, `/snap/bin`, `/var/lib/flatpak/exports/bin`,
      `~/.local/share/flatpak/exports/bin`. First hit wins.
- [x] 3.6 macOS implementation: check `/Applications/<Bundle>.app` and `~/Applications/<Bundle>.app`;
      the executable path is the bundle's `Contents/MacOS/<binary>` when it exists, else the bundle path.
- [x] 3.7 De-duplicate the result by `Id` and return a stable order (Chromium family first, then Gecko, each
      alphabetical) so the UI list does not reshuffle between opens.
- [x] 3.8 **The privacy boundary is the point of this service.** Add
...
- [x] 3.9 Extend `Unit/NoShellSpawnTests`' scanned set if the new files are not already covered by its
...
- [x] 3.10 Build with `-t:Rebuild`, 0 warnings. Run the filtered tests. Commit.

## 4. `ExtensionCatalogService` — fetch and gate the catalog

- [x] 4.1 Read `src/Downloader.Desktop/Services/PluginCatalogService.cs` in full. It is the template:
...
- [x] 4.2 Add `src/Downloader.Desktop/Models/ExtensionCatalogEntry.cs` mirroring the catalog JSON from
      task 2.3, plus a resolved `DownloadUrl`.
- [x] 4.3 Add `src/Downloader.Desktop/Services/ExtensionCatalogService.cs`: `FetchAsync(ct)` reads
...
- [x] 4.4 Add `Downloader.Desktop.Tests/Unit/ExtensionCatalogServiceTests.cs` driving the seam at a loopback
...
- [x] 4.5 Build (0 warnings), test, commit.

## 5. `ExtensionInstallService` — verify, then unpack, to a stable path

- [x] 5.1 Read `PluginManager.InstallFromZipAsync` — it is the download → **verify sha256 before extract** →
      install template, including the friendly error on mismatch.
- [x] 5.2 Add `src/Downloader.Desktop/Services/ExtensionInstallService.cs`. Expose `InstallRoot` (default
...
- [x] 5.3 `InstallAsync(ExtensionCatalogEntry entry, IProgress<double> progress, CancellationToken ct)`:
...
- [x] 5.4 `ReadInstalled(targetId)` returns the `installed.json` contents or null. Tolerate a missing or
      corrupt file (return null) — it is a convenience cache, not a source of truth.
- [x] 5.5 Clean up a stale `<TargetPath>.new` on the next install attempt.
- [x] 5.6 Add `Downloader.Desktop.Tests/Unit/ExtensionInstallServiceTests.cs` (loopback server + a zip built
...
- [x] 5.7 Add a test asserting **no path the service writes to is under any browser profile or extension
...
- [x] 5.8 Build (0 warnings), test, commit.

## 6. Report the extension's identity through the local API

- [x] 6.1 In `src/browser-extension/common.js`, add a pure helper that builds the identity pair —
...
- [x] 6.2 Attach it to the requests that already go out: query parameters on the GET forms
...
- [x] 6.3 Add unit tests to `common.test.js` for the helper: correct label per environment, correct version,
...
- [x] 6.4 In `src/Downloader.Desktop/Services/LocalApiService.cs`, add a pure parser for the identity (from
...
- [x] 6.5 A request carrying **no** identity must behave exactly as before. Add a test that pins this
      (an older extension and the CLI both send none).
- [x] 6.6 Add tests in `Downloader.Desktop.Tests/Unit` for the parser (all three carriers, malformed input
...
- [x] 6.7 Add a test asserting the reported identity does **not** appear in a saved `config.json` (mirror
      `Saving_the_config_keeps_the_referer_and_drops_cookies_and_headers`).
- [x] 6.8 Bump the extension version in **both** manifests (task 1.3's test enforces this) — a behaviour
...
- [x] 7.1 Add `src/Downloader.Desktop/ViewModels/ExtensionInstallViewModel.cs` following the repo's
...
- [x] 7.2 Expose one row per detected browser: `Name`, `Family`, `IsSelected`, plus
...
- [x] 7.3 Expose per-family `Mode`: `Store` when the family's catalog entry has a non-null `storeUrl`,
...
- [x] 7.4 Expose `InstallCommand`, `CopyPathCommand`, `OpenFolderCommand`, `OpenStoreCommand`, plus
      `IsBusy`, `Progress`, and `ErrorMessage`. `IsBusy` disables the install action.
- [x] 7.5 When a family's catalog entry is missing or excluded by `minAppVersion`, the row says the app
      needs updating and its install action is disabled — it must not silently offer nothing.
- [x] 7.6 `OpenStoreCommand` launches **that browser's own executable at the store URL**, via
...
- [x] 7.7 Add `Downloader.Desktop.Tests/Unit/ExtensionInstallViewModelTests.cs`: store mode chosen when a
...
- [x] 7.8 Build (0 warnings), test, commit.

## 8. `ExtensionInstallView` + Settings entry point

- [x] 8.1 Add `src/Downloader.Desktop/Views/ExtensionInstallView.axaml(.cs)` as a modal window in the repo's
...
- [x] 8.2 Lay out: a browser list (checkbox + name + family + a Connected dot mirroring the existing
...
- [x] 8.3 Add the entry point to `src/Downloader.Desktop/Views/SettingView.axaml` **inside the existing
...
- [x] 8.4 Add `DialogHelper.ShowExtensionInstall()` following the existing entry points, and **call
...
- [x] 8.5 Add all new i18n keys to **`en.json` first (it is the fallback), then all 16 packs** in
...
- [x] 8.6 Use `{i18n:Tr Key}` in XAML (never a literal string) and `Localizer.Instance["Key"]` in the VM.
      Any VM-computed localized string must subscribe to `Localizer.PropertyChanged` and re-raise, with an
      unsubscribe on teardown.
- [x] 8.7 Add to `Downloader.Desktop.Tests/UI`: the view loads (extend `UI/ViewLoadTests` if that is where
      ...
      `AppLifetime.Windows`.
- [x] 8.8 Add an i18n completeness test if one does not already exist for this dialog's keys: every new key
      present in every pack.
- [x] 8.9 Build (0 warnings), test, commit.

## 9. Wire it up and document it

- [x] 9.1 Register whatever needs DI in `App.axaml.cs` → `ConfigureServices()`, following the existing
...
- [x] 9.2 On startup, alongside the existing app-update and plugin-update checks in `MainViewModel`, check
...
- [x] 9.3 Update `README.md` → "Browser extension" → the **Install** block (currently "store listings
...
- [x] 9.4 Update `src/browser-extension/README.md` to mention the in-app installer as the recommended route.
- [x] 9.5 Add a short "Extension install" section to `docs/codebase-index.md` naming the new files and
      "where to change what", per the repo's standing rule that the index stays current.
- [x] 9.6 Commit.

## 10. Close out

- [x] 10.1 `dotnet build Downloader.Desktop.sln -t:Rebuild --nologo` from `src/` → **`0 Warning(s)`**, 0 errors.
      *Verified: 9 projects, 0 errors, 0 warnings. (One run died with `Internal CLR error (0x80131506)` — the
      known transient in SKILL.md; a plain retry was clean.)*
- [x] 10.2 Bounded full suite → **1490 passed, 0 failed** (1 m 54 s).
      *First run had 1 failure and it was mine: `A_successful_install_records_the_path_and_clears_busy` used
      `Task.Delay(200)` after a fire-and-forget `InstallCommand.Execute(null)`, which only breaks under
      full-suite load. Fixed by typing the command `ReactiveCommand<Unit, Unit>` and awaiting it — not by
      re-running. No sleeps left in that file.*
- [x] 10.3 `node --test src/browser-extension/common.test.js` → **134 passed**.
- [x] 10.4 `npx playwright test --workers=1` → **26 passed**.
      *First run failed `interception.spec.js` "the browser's copy is cancelled" (`in_progress` instead of
      `interrupted`). Investigated rather than re-run blindly, because that symptom is exactly what a
      REJECTED hand-off looks like and this change adds fields + a header to that POST: `startStubApp`
      answers 201 regardless of body, and the spec passes 10/10 alone, so it is the documented port/context
      contention — which can bite even at `--workers=1`. Noted in SKILL.md.*
- [x] 10.5 Screenshots regenerated and **viewed**. Added `settings-extension-{dark,light}.png` — a new
      scrolled capture locating the row by its button, since the new Settings row sits below the fold and
      was in none of the existing shots. md5 differs from `settings-logging-dark.png`, so the scroll really
...
- [ ] 10.6 **NOT DONE — cannot be verified from this machine, and is the author's to run.** The real
      ...
      Also unverifiable here and equally unclaimed: **Windows registry detection** and **macOS `.app`
      bundle detection** (`BrowserDetector.FindOnWindows` / `FindOnMac`) — written fail-soft, with the pure
      part (`UnquoteCommand`) unit-tested on every platform, but never executed on those OSes.
- [x] 10.7 Boxes ticked against real output only; 10.6 left open with the reason above.
- [x] 10.8 SKILL.md updated: why elevation is settled, the stable-path rule, the detection boundary, the
      `storeUrl` switch, the identity handshake — plus three traps this session hit (`git commit` commits
      the INDEX on a shared checkout; the interception spec flakes at `workers=1`; a `Task.Delay` after a
      fire-and-forget command execution is a CI flake).
- [x] 10.9 `/opsx:sync` the delta specs into `openspec/specs/`, then `/opsx:archive`. Push `develop`.

...
- [ ] 1.1 Add a pure `ParseCookieHeader(string cookieHeader, string targetUrl)` to `LocalApiService`
...
- [ ] 1.2 Add a pure `ParseHeaderBlock(string block)` that turns a newline-separated `Name: value`
      block into a case-insensitive dictionary, skipping malformed or empty-named lines.
- [ ] 1.3 Wire both into `ApiAddRequest.FromQuery` (`cookies`, `headers`), leaving the existing
      `referer` handling as-is. A parse problem must never fail the add.
- [ ] 1.4 Unit-test both parsers directly: multiple pairs, values containing `=`, stray whitespace,
      a trailing `;`, an empty string, garbage with no pair, and a header block with a blank line.
- [ ] 1.5 Unit-test `FromQuery` end to end — a query carrying cookies + headers + referer produces a
      request whose `BuildItem` yields an item with all three on `item.Request`.

## 2. Tell the caller what was accepted

- [ ] 2.1 Extend the `/api/add` `201` body with `cookies` (count), `headers` (count) and `referer`
      (bool) alongside `id`. Counts only — never any value.
- [ ] 2.2 Test that the counts reflect what was actually accepted, including the zero case, and that
      no cookie or header value appears anywhere in the response.
- [ ] 2.3 Confirm no code path logs the request URL or query string; keep `LocalApiService`'s error
...
- [ ] 3.1 In `DownloadManager.Plans.cs`, track the set of live part engines and expose it (or a
      pause/resume callback) to the manager instead of publishing only the most recent one.
- [ ] 3.2 Add an `isPaused` predicate to `ExecutePlanAsync` beside `isCancelled`; the runner must not
...
- [ ] 3.3 Make `DownloadManager.Pause` pause every live part engine, and `Resume` resume them, for a
      plan-backed row. Keep the existing state guards (pause only from Running, etc.).
- [ ] 3.4 Test with the loopback plan-runner harness: pause a multi-segment plan mid-run, assert no
...
- [ ] 3.5 Test that a paused plan does not keep accumulating bytes behind a frozen progress display
...
- [ ] 4.1 Add an optional header map to `ConcatRecipe` (absent ⇒ current behavior) and make sure it
      round-trips through `PersistedPlan` with older recipes deserializing unchanged.
- [ ] 4.2 Fold the download's cookies into the `ResolveOptions.Headers` bag as a single `Cookie:`
      header when `DownloadManager` builds them, so a resolver can pass them on.
- [ ] 4.3 Have `HlsResolver` stamp the request headers onto the `ConcatRecipe` it produces, and
      `HlsPostProcessor` send them on the key request instead of using a bare `HttpClient`.
- [ ] 4.4 Test with a loopback server that serves the key only when the expected cookie/referer are
      present: the stream assembles with a context and fails without one.
- [ ] 4.5 Bump the HLS plugin csproj `<Version>` (standing rule — a stale version means the catalog
      update check never offers the fix).

## 5. Docs, verification and follow-up

- [ ] 5.1 Document the GET query's `cookies`/`headers` forms in `docs/local-api.md`, with a worked
      example, and say plainly that POST is preferred and why (secrets in URLs).
- [ ] 5.2 Note the new `201` response fields in the same doc.
- [ ] 5.3 Full solution build with `-t:Rebuild` reporting `0 Warning(s)`, and a green bounded
      `dotnet test` run.
- [ ] 5.4 Append the non-obvious findings to `.claude/skills/downloader-desktop/SKILL.md` — the

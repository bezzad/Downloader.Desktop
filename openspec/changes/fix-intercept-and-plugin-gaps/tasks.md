...
- [ ] 1.5 Add a Playwright e2e case serving an extensionless URL whose `Content-Disposition` names a `.xapk`, and assert the extension hands it to the stub app instead of letting the browser keep it.
- [ ] 1.6 Verify the whole group against the real regression: `node --test src/browser-extension/common.test.js` green, `npm test` in `e2e/` green, and the tests from 1.1 now passing.

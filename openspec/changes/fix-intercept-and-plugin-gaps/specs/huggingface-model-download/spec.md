## Purpose
Downloading an AI model from a HuggingFace repository — choosing which quantisation of a GGUF model to
fetch, downloading it like any other file, and adding it to the local Ollama store afterwards — so a model
page a user found on HuggingFace is as downloadable as an ollama.com one.

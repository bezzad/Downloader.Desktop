...
- [ ] 7.3 Manual end-to-end check (after host Phase-2 wiring): paste an x.com video URL in the app → it downloads and plays. Note the result in this change before archiving.
  ...
  > 2026-07-06 (update — blocker resolved): Phase-2 IS landed (`DownloadManager.Plans.cs` multi-part plan runner) AND the HLS plugin is now **in-repo** (optional/catalog tier) via the `consolidate-official-plugins` change — no separate repo, no separate release needed. Only the author's manual check remains: install the HLS plugin (Settings → Plugins → More plugins → Add, once a release with the catalog exists; or drop the built `Downloader.Desktop.Plugins.Hls` DLL into `~/.config/Downloader/plugins`), paste an x.com / YouTube URL, confirm it downloads + plays, then record the result and archive. Tracked as task 9.1 in `consolidate-official-plugins`.

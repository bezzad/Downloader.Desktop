## Purpose
Resolves every download to exactly one file-type category (Video, Audio, Image, Document, Archive,
App, Disc, Other, plus user-created ones), with a fixed precedence (explicit choice, then
extension, then Content-Type, then Other), and lets the user manage the category list itself.
...
#### Scenario: A query string does not defeat extension detection
- **WHEN** a download's URL is a signed link whose file name is followed by a query string, e.g.
  `movie.mkv?md5=...&expires=...`, and no override or resolved file name says otherwise
- **THEN** it resolves to the Video category from the `.mkv` extension, not to Other
...
#### Scenario: The Add dialog's automatic suggestion follows the links being edited
- **WHEN** the user pastes one link, then replaces it with a batch of several links (or edits the
  links after the dialog opened)
- **THEN** the automatic entry's suggestion updates to match the first link currently in the box,
  not whatever it showed when the dialog opened

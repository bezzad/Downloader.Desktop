# download-archive Specification

## Purpose

Taking a download out of the working list without destroying it. Archiving is a separate axis from a
download's state, not a seventh status: an archived download keeps its record, its file and its
state, and reads as Failed or Completed again the moment it is restored.
...
### Requirement: Archived downloads take no part in queues or live totals
Archived downloads SHALL be excluded from the queue concurrency pump, from bulk start/stop actions,
from the Queues page listings and counts, and from the status bar's total speed. They SHALL still be
counted in the status bar's cumulative downloaded total, which measures what the app has fetched
rather than what the working list currently holds.
...
#### Scenario: Archiving does not shrink the downloaded total
- **WHEN** a download with downloaded bytes is archived
- **THEN** the status bar's cumulative downloaded total is unchanged, while the total speed excludes it

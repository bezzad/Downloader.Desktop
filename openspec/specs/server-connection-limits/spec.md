# server-connection-limits Specification

## Purpose
Discovering how many simultaneous connections a host actually accepts, remembering it, and reusing it — so
the user's configured count is a ceiling rather than a number every server must agree to, and the cost of
learning a host's limit (a refused attempt and the partial file it discards) is paid once per host rather

...
- [ ] **Use the engine's new `RemoteFileResolver` for the downloads grid (filename + size without downloading).** The `Downloader` engine (on its `develop`) now publicly exposes file-metadata resolution so we no longer need to hand-roll it or start/stop a download to learn a queued item's name/size: `RemoteFileResolver.GetFileNameAsync(url[, config, ct])` and `RemoteFileResolver.GetFileInfoAsync(url[, config, ct])` → `RemoteFileInfo { FileName, FileSize (-1 if unknown), SupportsRange, Address (final, post-redirect) }`. It uses the same logic the engine uses internally (`Content-Disposition` → URL path → GUID; size from `Content-Range`/`Content-Length`) via the canonical `SocketClient.GetFileInfoAsync`. Same lookup is also on the service: `IDownloadService.GetFileInfoAsync(url, ct)`. **Action:** replace our hand-rolled `src/Downloader.Desktop/Services/UrlResolver.ResolveFileNameAsync` with `RemoteFileResolver.GetFileInfoAsync`, and use `info.FileSize` to **also show the size** in the main downloads Grid for queued/waiting items (today we only preview the name — see `DownloadItemViewModel._previewName`/"Fetching name…", `DownloadsViewModel`). Keep our `NameGate` concurrency cap + short timeout wrapper around the call (don't let a slow server stall the UI). **Prerequisite / BLOCKER:** we reference `Downloader` **NuGet 5.8.1** (`Downloader.Desktop.csproj`), but `RemoteFileResolver` is unreleased (engine `develop` only, 5.8.1 has the version unchanged) — bump the `PackageReference` to the first published engine version that includes it before doing this. Engine README documents the API under "How to get the file name and size without downloading".

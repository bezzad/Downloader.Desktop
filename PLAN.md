...
**Branch**: develop
**Now working on**: Issue 3 — Instagram video download R&D (plan only); v1.1.3 (Queues) release building
...
## Active
- [~] **Issue 3 — Instagram video download R&D (plan only, per author).** Deliver a written plan + trade-offs in `docs/instagram-rnd.md`; no code. Engine fetches direct HTTP+range URLs only — IG needs a media extractor.

## Todo
- [ ] (after CI) Update tap cask to v1.1.3 + brew upgrade + verify Queues redesign live on this Mac.

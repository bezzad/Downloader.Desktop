...
- `Downloader.Desktop.Plugins/` — the first-party plugins, in **two tiers** (all first-party plugin source now lives in THIS repo — the former separate `bezzad/Downloader.Plugins` repo was consolidated in; it is deleted only after the author confirms this works):
  ...
  - **OPTIONAL / catalog tier** (NOT bundled, NOT referenced by the app, absent on a fresh install; installed on demand from Settings → Plugins): `Downloader.Desktop.Plugins.Hls` (HLS/`.m3u8` segment download + ffmpeg remux, quality picker from master playlists; id `com.bezzad.hls`) and `Downloader.Desktop.Plugins.Website` (save a page/site as an offline-browsable `.zip`; id `com.bezzad.website-zip` — offers an "Offline copy (.zip)" Add-dialog variant on `text/html` links via the `websitezip:` scheme; its crawl runs through the app's `ITransfer` path, `DownloadManager.Transfers.cs`; requires app ≥ 2.1.0, enforced by the catalog's `minAppVersion`) and `Downloader.Desktop.Plugins.SiteMedia` (a video-site page → the real media stream(s) via yt-dlp, with a per-quality picker; id `com.bezzad.site-media`. **The only component that runs a third-party binary**: fetched on first use, sha256-verified against the digest its publisher lists BEFORE it is made executable, run from an absolute path with no shell, and it reads NO browser profile or cookie store — a signed-in session reaches it only as the cookie file our extension captured. The popup asks the app `/api/can-handle` before declaring a site unsupported, so a user with this plugin is offered the page and one without is told which plugin would do it). They're in the solution for build/test only. At release time `scripts/build-plugins.sh` (run by `release.yml`) zips it + generates `plugins-catalog.json` (from `packaging/plugins/optional-plugins.json` + the built version/sha256) and attaches both to the same `vX.Y.Z` release. The app fetches that catalog (`Services/PluginCatalogService`), shows uninstalled ones under "More plugins", and on **Add** downloads → **verifies sha256 before load** (`PluginManager.InstallFromZipAsync`) → installs into `PluginsRoot` as a normal removable plugin. Update checks compare installed vs catalog version and prompt (never auto). Isolation is guarded by `PluginIsolationTests` + a `release.yml` publish grep. Its own version lives in its csproj `<Version>` (single source; `HlsPlugin.Version` derives from the assembly).
...
14. ✅ **Issue #9 follow-up + plugin gaps (`fix-intercept-and-plugin-gaps`)** (DONE):
   - **Interception type detection**: a download's type is judged from EVERY source that names it (browser file name, response `Content-Disposition` — cached from the existing `onHeadersReceived` listener, no new permission — URL query, path, MIME), not the first non-empty one; an implausible path "extension" (`…/XAPK/com.instagram.android`) contributes nothing. Extension 1.6.0.
   - **Hand-off**: the CLICKED link travels as the download's primary URL with the redirect chain's signed end as a mirror, so the app re-walks the chain instead of re-requesting a spent single-use address; an extension hand-off earns one auto re-resolve from zero bytes and never reads "paste a fresh link" for a file the browser still has.
   - **New optional plugin `com.bezzad.site-media`** (see the tier list above) + `/api/can-handle` so the extension's unsupported-site state depends on what the running app can actually do. A resolver that CLAIMS a link and then refuses it now fails the row with its own reason instead of being swallowed and the page fetched as an ordinary URL.
   - **Ollama**: the "Add to Ollama" offer was lost on the TRANSFER completion route (the owning plugin id was only ever recorded for a *resolver*); all three routes + a restart are now pinned by tests. **HuggingFace model repos** are claimed by the Ollama plugin (1.2.0): GGUF files offered as quantisation variants, installed under `hf.co/<owner>/<repo>:<quant>`, verified against the digest the repository publishes.

15. ✅ **v2.8.0 regression fix (`fix-handoff-url-and-server-limits`)** (DONE):
   - **Mirrors are load spreading, NOT failover** — the fact v2.8.0's hand-off change got wrong. The engine pins each chunk to one url and probes with `Urls[0]` only, so the "fallback" the extension started sending was never requested and Softpedia's external mirror / zip and APKMirror all broke. Failover is now the app's (`DownloadManager.TryNextUrl`/`OrderUrlsForAttempt`), one leading attempt per address; the extension leads with the redirect chain's end again (`handOffUrls` in `common.js`, extension 1.6.1).
   - **A server that refuses concurrency** (the reporter measured 403 from the 4th connection on one mirror) gets one single-connection retry — which must also DISCARD the partial file, because a resumed download keeps its original chunk layout — and a 403 that survives it says so (`Error_ServerRefusedConnections`) instead of claiming the link expired.
   - Engine gotchas found while proving it are in the skill file: the engine mutates the `DownloadConfiguration` you give it, the size probe is `GET bytes=0-0` (not HEAD), and `Completed` precedes the file being in place.

16. ✅ **Round 14 — fix non-working install commands** (DONE):
